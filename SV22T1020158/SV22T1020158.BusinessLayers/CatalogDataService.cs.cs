...
namespace SV22T1020158.BusinessLayers
{
    ...
    public static class CatalogDataService
    {
        ...
        public static async Task<int> AddProductAsync(Product data)
        {
            ...
            // Kiểm tra chặt chẽ nullable int
            if (data.CategoryID == null || data.CategoryID <= 0) return 0;
            if (data.SupplierID == null || data.SupplierID <= 0) return 0;
        ...
        {
            //Kiểm tra dữ liệu hợp lệ
            if (data.ProductID <= 0) return false;
            ...
            if (data.CategoryID == null || data.CategoryID <= 0) return false;
            if (data.SupplierID == null || data.SupplierID <= 0) return false;
        ...
        {
            return await productDB.GetPhotosAsync(photoID);
        }
        ...
        public static async Task<long> AddPhotoAsync(ProductPhoto data)
        {
            //Kiểm tra dữ liệu hợp lệ
            if (data.ProductID <= 0) return 0;
            if (string.IsNullOrWhiteSpace(data.Photo)) return 0;
        ...
        {
            //Kiểm tra dữ liệu hợp lệ
            if (data.PhotoID <= 0) return false;
            if (data.ProductID <= 0) return false;
            if (string.IsNullOrWhiteSpace(data.Photo)) return false;
        ...
        {
            //Kiểm tra dữ liệu hợp lệ
            if (data.CategoryID <= 0) return false;
        ...
        // Thêm vào CatalogDataService.cs
        // Thay vì Task<List<Product>>
        public static async Task<PagedResult<Product>> ListProductsForShopAsync(ProductSearchInput input)
        {
            return await productDB.ListForShopAsync(input);
        }

        public static async Task<Product?> GetProductDetailAsync(int productID)
{
    return await productDB.GetAsync(productID);
}
    }
